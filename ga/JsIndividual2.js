...
import JsMutations from "./JsMutations2.js";
...
class JsIndividual extends GenericIndividual {
  ...
  chooseMutation(node) {
    if (Math.random() <= 0.999) {
      let mutationsList = this.mutations;
      let chosenMutation = mutationsList[Math.floor(Math.random() * mutationsList.length)];
      mLogger(`Performing ${chosenMutation.name} mutation for ${node.type}...`);
      chosenMutation(node);
